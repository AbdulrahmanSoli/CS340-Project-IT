...
# Query 3 - assign an asset (admin only, assignmentID auto-generated by DB)
@assignments_bp.route('/assignments/add', methods=['POST'])
def assign_asset():
    if admin_required():
        return redirect('/login')
    data = (request.form['asset_id'], request.form['user_id'], session['user_id'])
    result = query('INSERT INTO asset_assignment (assignedDate, assetID, userID, assignedBy) VALUES (CURRENT_DATE, %s, %s, %s)', data)
    if result is None:
        rows = query('SELECT * FROM asset_assignment WHERE returnDate IS NULL')
